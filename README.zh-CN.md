...
**性能指标**: 最短周期 **~1 μs**（x86）/ **~4 μs**（ARM64）| 最短抖动 **~0.1 μs** | 典型周期 ~100 μs | 热路径 &lt; 0.5% | L2Bus 循环 ~20 μs（x86，含创建开销）

**[English](README.md)** | **[API 文档](docs/api/)** | **[性能报告](docs/performance.html)** | **[重构报告](docs/)**
...
软件下限为预创建 bus/slave 后的 `New→fill→Cycle`（不含网卡/OS 调度）。完整方法与表格见 [性能极限报告](docs/performance.html)。
...
完整性能极限数字请参见 [性能报告](https://anviod.github.io/EtherCAT/performance.html)；架构与重构说明见 [重构报告](https://anviod.github.io/EtherCAT)。
