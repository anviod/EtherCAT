# EtherCAT — Go 语言工业以太网协议 SDK

[![CI](https://github.com/anviod/EtherCAT/actions/workflows/ci.yml/badge.svg)](https://github.com/anviod/EtherCAT/actions/workflows/ci.yml)
[![Go Version](https://img.shields.io/badge/Go-%3E%3D1.21-blue)](https://go.dev/)
[![License](https://img.shields.io/badge/License-MIT-green.svg)](LICENSE)
[![Tests](https://img.shields.io/badge/tests-225%2B-brightgreen)](.)
[![Coverage](https://img.shields.io/badge/coverage-74%25--92%25-brightgreen)](.)
[![Docs](https://img.shields.io/badge/docs-GitHub%20Pages-2563eb)](https://anviod.github.io/EtherCAT)

面向工业控制、边缘计算和设备厂商的**高性能 EtherCAT SDK**，提供从 ESI 解析、主站通信、从站模拟到测试验证的完整工具链。纯 Go 实现，零外部运行时依赖，可在 x86-64 / ARM64 平台上直接部署。

**[English](README.md)** | **[API 文档](docs/api/)** | **[性能报告](docs/performance.html)** | **[重构报告](docs/)** | **[路线图](ROADMAP.zh-CN.md)**

## 适用场景

- **工业网关**：在边缘侧解析 EtherCAT 帧，桥接 MQTT / OPC UA / HTTP 等上层协议
- **EtherCAT 主站**：构建纯 Go 主站运行时，驱动伺服、IO、传感器等从站设备
- **从站模拟**：自动化测试中模拟 EtherCAT 从站行为，无需真实硬件
- **自动化测试**：CI 中集成从站模拟，验证主站逻辑的正确性与性能
- **协议分析**：解析、构造、验证 EtherCAT 帧与数据报，用于调试与诊断
- **CI 仿真**：在 GitHub Actions 等 CI 环境中运行协议级回归测试

## 特性

- [x] **纯 Go 实现** — 零 CGo，零内核模块，跨平台编译与部署
- [x] **零分配热路径** — 编解码核心路径 0 B/op, 0 allocs/op
- [x] **225+ 测试用例** — 覆盖率 74%–92%，含竞态检测与压力测试
- [x] **ESI XML 解析** — 完整的 EtherCAT Slave Information 文件解析
- [x] **帧/数据报编解码** — 高性能 Frame、Datagram、Ethernet 帧处理
- [x] **命令执行引擎** — goroutine 安全的多路复用命令调度
- [x] **EEPROM 访问** — ESC EEPROM 读写操作
- [x] **L2 从站/总线模拟** — 无需真实硬件即可进行协议级测试
- [x] **UDP 多播传输** — 基于 UDP 的 EtherCAT 传输层
- [x] **CLI 工具** — 命令行解析、扫描、读写操作
- [x] **GitHub Actions CI** — 多 Go 版本矩阵测试 + 基准测试 + 代码检查
- [x] **中英双语文档** — README + API 文档 + 性能报告 + 路线图

## 架构

```
ecad (ESC 寄存器地址常量)
  ↓
ecfr (帧/数据报编解码) ← internal/marshalling (共享编解码器)
  ↓
ecmd (命令执行、帧调度、并发复用)
  ↓
┌──────────┬──────────┬──────────────────┐
│ etransport│  ecee    │  internal/sim    │
│ (UDP)     │ (EEPROM) │  (从站模拟)      │
└──────────┴──────────┴──────────────────┘
eni (ESI XML 解析器) — 独立模块
```

## 生态位置

```
┌─────────────────────────────────────┐
│           工业应用层                 │
│    PLC Runtime / Motion / Safety    │
├─────────────────────────────────────┤
│         EtherCAT Master             │  ← 下一阶段目标
│   AL State Machine / DC / Mailbox   │
├─────────────────────────────────────┤
│      EtherCAT Protocol SDK          │  ← 当前定位 (v1.0.3)
│   Frame / Command / ESI / EEPROM    │
├─────────────────────────────────────┤
│   边缘网关 / 工业协议转换 / 仿真测试   │
└─────────────────────────────────────┘
```

## 项目结构

```
├── cmd/ethercat/          # CLI 命令行工具
├── docs/                  # GitHub Pages + API 文档
│   ├── api/               # 生成的 Go 包文档
│   └── index.html         # 重构报告
├── ecad/                  # ESC 寄存器地址常量
├── ecee/                  # EEPROM 访问
├── ecfr/                  # 帧/数据报编解码
├── ecmd/                  # 命令执行
├── eni/                   # ESI XML 解析器
├── etransport/            # UDP 多播传输层
├── internal/              # 私有包
│   ├── marshalling/       # 小端/大端编解码辅助
│   └── sim/               # L2 从站/总线模拟
├── .github/workflows/     # CI（测试 + 部署 Pages）
├── go.mod
├── Makefile
├── README.md
└── README.zh-CN.md
```

## 包说明

| 包 | 描述 |
|---------|-------------|
| `ecad` | ESC 寄存器地址常量定义 (ETG.1000 规范) |
| `ecfr` | EtherCAT 帧、数据报和以太网帧编解码 |
| `ecmd` | 命令执行、帧调度和 goroutine 安全的多路复用 |
| `ecee` | ESC EEPROM 读写访问 |
| `eni` | ESI（EtherCAT 从站信息）XML 文件解析器 |
| `etransport` | UDP 多播传输层（实现 ecmd.Framer 接口） |
| `internal/sim` | L2 从站和总线模拟，用于测试 |
| `internal/marshalling` | 共享的小端/大端二进制编解码辅助函数 |

## 快速开始

### 安装

```bash
go get github.com/anviod/EtherCAT@v1.0.3
```

### 解析 ESI 文件

```go
import "github.com/anviod/EtherCAT/eni"

info, err := eni.ReadEtherCATInfoFromFile("slave.xml")
if err != nil {
    panic(err)
}
fmt.Printf("厂商: %s\n", info.Vendor.Name)
```

### 创建并编码数据报

```go
import "github.com/anviod/EtherCAT/ecfr"

buf := make([]byte, 128)
frame, _ := ecfr.PointFrameTo(buf)

dg, _ := frame.NewDatagram(4)
dg.Header.Command = ecfr.APRD
addr := ecfr.PositionalAddr(0, 0x1000)
dg.Header.Addr32 = addr.Addr32()
dg.Header.SetLast(true)

frame.Commit()
```

### 执行读写命令

```go
import (
    "github.com/anviod/EtherCAT/ecfr"
    "github.com/anviod/EtherCAT/ecmd"
)

cf := ecmd.NewCommandFramer(framer)
addr := ecfr.PositionalAddr(0, 0x0000)
typ, err := ecmd.ExecuteRead16(cf, addr, 1)
```

### 运行测试

```bash
# 全部单元测试
make test

# 含覆盖率
make test-cover

# 竞态检测
make test-race

# 基准测试
make bench

# 压力测试
make bench-stress
```

## Bug 修复 (v1.0.1)

| # | 包 | 严重度 | 问题 | 修复 |
|---|---------|----------|-------|-----|
| 1 | `ecee` | 严重 | EEPROM 忙时无限循环 | 使用 `time.After` + `select` 超时机制 |
| 2 | `ecfr/eth` | 严重 | EtherType 高字节被覆盖 | 修正 pos/pos+1 写入顺序 |
| 3 | `link/udp` | 严重 | `Close()` 栈溢出 | 修复递归调用 `sock.Close()` |
| 4 | `ecfr/frame` | 中等 | 超大数据报导致 `panic` | 改为返回 `error` |
| 5 | `sim/l2eeprom` | 中等 | uint8 移位溢出 | 使用 uint32 类型转换 |
| 6 | `ecfr/datagram` | 中等 | 静默吞掉错误 | 传播所有子组件错误 |

## 性能

> **注意**：以下数字代表软件栈内部处理性能（编解码、帧操作、命令执行），
> 并非端到端 EtherCAT 网络实时周期。实际系统周期还取决于 NIC 驱动、IRQ 延迟、
> DMA 传输、操作系统调度等因素。完整方法与数据见 [性能极限报告](docs/performance.html)。

**测试环境**：x86-64 平台为 Intel i5-13500H + Go 1.23 + Windows 11；ARM64 平台为 Rockchip RK3588s + Go 1.23 + Linux (ARMBIAN)。所有编解码热路径均**零分配**（0 B/op, 0 allocs/op）。

### 对外承诺指标（基准实测）

| 指标 | x86-64 (i5-13500H) | ARM64 (RK3588s) | 测量依据 |
|------|--------------------|-----------------|----------|
| **最短周期** | **~1 μs**（min 0.76 μs） | **~4 μs**（min 3.70 μs） | `TestL2BusCycleJitter` / `BenchmarkL2BusSteadyCycle` |
| **最短抖动** | **~0.1 μs**（stddev 66 ns） | **~0.1 μs**（stddev 108 ns） | 多次 `ns/op` 标准差 |
| 典型 EtherCAT 周期 | ~100 μs | ~100 μs | 工业规划目标 |
| 热路径占 100 μs 比 | &lt; 0.5% | &lt; 0.5% | DatagramHeader Overlay+Commit |
| L2Bus.Cycle（含创建） | ~20 μs | ~70 μs | `BenchmarkL2BusCycle` |

软件下限为预创建 bus/slave 后的 `New→fill→Cycle`（不含网卡/OS 调度）。

### 端到端微秒级循环时间分析

EtherCAT 典型循环时间要求为 100μs 到 1ms。本库的端到端处理管线性能如下：

| 阶段 | 耗时 | 占 100μs 周期比 | 内存分配 |
|------|------|-----------------|----------|
| 数据报头编解码 | 2–5 ns | &lt; 0.01% | 0 B/op |
| 帧覆盖/提交 | 60–770 ns | &lt; 0.8% | 104 B/op |
| **最短稳态总线循环** | **~1 μs (x86) / ~4 μs (ARM)** | **~1–4%** | 10 allocs/op |
| L2Bus.Cycle（含创建） | ~20 μs (x86) | ~20% | 21 allocs/op |
| 命令执行 | 250–326 ns | &lt; 0.4% | 32–296 B/op |

**核心热路径（编解码 + 帧操作 + 命令执行）在 100μs EtherCAT 典型周期中占比远低于 0.5%。** 10 字节数据报头是系统中最高频的操作，通过仅 2 次内存操作（一次 `uint64` 8 字节 + 一次 `uint16` 2 字节）替代 10 次逐字节访问，Commit 路径提升 21%。

### 核心编解码 (ecfr) — 全部零分配

| 基准测试 | 耗时 | 技术手段 |
|-----------|------|-----------|
| `DatagramHeader.Overlay` | **~3.0 ns** (x86) / ~7.8 ns (ARM) | 单次 `uint64` 读取(8字节) + `uint16` 读取(2字节) |
| `DatagramHeader.Commit` | **~2.1 ns** (x86) / ~4.9 ns (ARM) | 单次 `uint64` 写入 + `uint16` 写入 |
| `Datagram.Overlay` (32B) | ~5.0 ns | 头部覆盖 + WKC 通过 `unsafe.Pointer` |
| `Datagram.Overlay` (最大 2047B) | ~5.0 ns | 常量时间，与数据长度无关 |
| `Header.Overlay/Commit` | ~0.5 ns | 单次 `uint16` 读写 |
| `ETHFrame.WriteDown` | **~2.1 ns** | `binary.BigEndian` 处理网络字节序字段 |

### 帧操作 (ecfr)

| 基准测试 | 耗时 | 分配 |
|-----------|------|--------|
| `Frame.Overlay` (单数据报) | **~78 ns** | 2 次 / 104 B |
| `Frame.Overlay` (3 数据报) | ~280 ns | 6 次 / 344 B |
| `Frame.NewDatagram` | ~180 ns | 2 次 / 64 B |
| `FullPipeline` (提交+解析+验证) | **~430 ns** | 2 次 / 104 B |

### 从站模拟 (sim)

| 基准测试 | 耗时 | 技术手段 |
|-----------|------|-----------|
| `L2Slave.ProcessFrame` (4B) | **~270 ns** | 非寄存器区域批量 `copy()` |
| `L2Slave.ProcessFrame` (100B) | ~332 ns | 批量 `copy()` — 1次调用替代100次逐字节 |
| `L2Slave.ProcessFrame` (1KB) | ~776 ns | 批量 `copy()` — 1次调用替代1000次逐字节 |
| `L2Slave.ProcessFrame` (寄存器) | ~357 ns | 逐字节分发到设备映射 |
| `L2Bus.SteadyCycle` | **~1 μs** (x86) / **~4 μs** (ARM) | 最短软件周期（预创建总线） |
| `L2Bus.Cycle` | **~20 μs** (x86) / ~70 μs (ARM) | 含创建开销的提交→复制→解析→处理 |
| `L2Bus.FullPipeline` | ~66–75 μs (ARM) | 创建→填充→循环→验证 |

### 命令执行 (ecmd)

| 基准测试 | 耗时 | 分配 |
|-----------|------|--------|
| `ExecuteRead8` | ~250 ns | 7 次 / 296 B |
| `ExecuteWrite8` | ~300 ns | 7 次 / 296 B |
| `CommandFramer.Cycle` | ~356 ns | 1 次 / 32 B |
| `BatchReadWrite` (3 命令) | **~326 ns** | 1 次 / 32 B |

### 关键优化 (v1.0.3)

| # | 优化项 | 效果 |
|---|-------------|--------|
| 1 | 10字节数据报头采用 `uint64`+`uint16` 单次读取 | Overlay: 5.3→5.0 ns, Commit: 4.3→3.3 ns（**提升 21%**） |
| 2 | 从站模拟中非寄存器内存批量 `copy()` | 100B 读取：100→1 次函数调用（减少 100 倍） |
| 3 | `Frame.ByteLen()` O(1) 缓存 | 消除 Commit/NewDatagram 中的 O(n) 遍历 |
| 4 | 热路径中用哨兵错误替代 `fmt.Errorf` | 错误路径零分配 |
| 5 | 写缓冲区栈分配（1/2/4 字节） | `ExecuteWrite8/16/32` 零堆分配 |

## 路线图

当前定位为 **Pure Go EtherCAT Protocol Library**，下一阶段目标是成长为符合 ETG 规范的工业级 EtherCAT Master。详细规划见 [路线图](ROADMAP.zh-CN.md)。

| 阶段 | 目标 | 核心能力 |
|------|------|---------|
| v1.0.4 | 核心协议栈 | AL 状态机 + 网络扫描 + PDO Mapping + CoE |
| v1.0.5 | 工业可靠性 | Watchdog + 错误恢复 + 长时间稳定性测试 |
| v1.0.6 | 高级特性 | DC + FoE + 诊断 + 拓扑可视化 |
| v1.1.0 | 验证与合规 | 真实设备互操作 + 现场试点 |

## 文档

完整性能极限数字请参见 [性能报告](https://anviod.github.io/EtherCAT/performance.html)；架构与重构说明见 [重构报告](https://anviod.github.io/EtherCAT)；项目规划见 [路线图](ROADMAP.zh-CN.md)。

## License

MIT — 详见 [LICENSE](LICENSE)。